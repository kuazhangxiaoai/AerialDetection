...
import torch #it is necessary to load libc10.so file
...
if __name__ == "__main__":
    module = RoIAlign((30,30),1)
    input = torch.randn([4,64,256,256])
    roi = torch.Tensor([[2, 35, 35, 65, 65]])
    module = module.cuda()
    input = input.cuda()
    roi = roi.cuda()
    out = module(input, roi)
    print("create")